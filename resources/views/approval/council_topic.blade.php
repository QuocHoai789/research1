...
<head>
    ...
    <title>Thành lập hội đồng nghiệm thu đề tài</title>
    <base href="{{ asset('') }}">
...
<body>
	<div class="dashboard-header ">
		...
		<nav class="navbar navbar-expand-lg bg-white fixed-top">
			<a class="navbar-brand" href="{{ route('admin') }}"><img class="logo-img" width="50" src="cntt/images/icons/logo.png" alt="logo"></a><span class="h7">Lập hội đồng nghiệm thu đề tài</span> 
					...
					<div class="row " style="padding-top: 20px;">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="page-header">
								<h2 class="pageheader-title">Tài liệu:{{$topic->name_topic}} </h2>
		...
		<div class="row">
			...
			<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog" role="document">
					<form id="form-approval-topic" method="post" action="{{ route('duyettailieu') }}">
                ...
                <div class="modal-dialog" role="document">
                   {{--  <form id="form-approval-topic" method="post" action="">
                        <input type="hidden" name="_token" value="{{csrf_token()}}">
                        <input type="hidden" id="id_topic" name="id_topic"> --}}
                        <div class="modal-content">
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                <table class="table">
                                    ...
                                    <tbody>
                                        @isset($list_council)
                                        @foreach($list_council as $key => $greed)
                                        <tr>
                                            <td>{{ $i++ }}</td>
                                            <td>{{ $greed }}</td>
                                            <td>
               ...
               {{-- Modal Cancel --}}
                ...
                <div class="modal fade" id="sendgreedcouncil" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <form id="form-send-approval" method="post" action="{{ route('guihoidongthamdinhdetai',$topic->id) }}">
                            <input type="hidden" name="_token" value="{{csrf_token()}}">
                            <input type="hidden" id="id_topic" name="id_topic" value="{{ $topic->id }}">
                            
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h3 class="modal-title font-weight-bold text-primary" id="exampleModalLabel"> </h3>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body">
                                      ...
                                      <div class="form-group">
                                    <label for="recipient-name" class="font-weight-bold col-form-label">Chủ tịch hội đồng:</label>
                                    
                                  <select id="select_state1" name="council[president]" class="form-control choose_mail"  required>
                                    ...
                                    <div class="form-group">
                                    <label for="recipient-name" class="font-weight-bold col-form-label">Ủy viên:</label>
                                    
                                  <select id="select_state1" name="council[uv]" class="form-control choose_mail"  required>
                                   ...
                                   <div class="form-group">
                                    <label for="recipient-name" class="font-weight-bold col-form-label">Ủy viên, phản biện 1:</label>
                                    
                                  <select id="select_state2" name="council[pb1]" class="form-control  choose_mail"  required>
                              ...
                              <div class="form-group">
                                    <label for="recipient-name" class="font-weight-bold col-form-label">Ủy viên, phản biện 2:</label>
                                    <select id="select_state2" name="council[pb2]" class="form-control  choose_mail"  required>
                              ...
                              <div class="form-group">
                                <label for="recipient-name" class="font-weight-bold col-form-label">Ủy viên, Thư ký hội đồng:</label>
                                 <select id="select_state4" name="council[tk]" class="form-control choose_mail" required>
                		...
                		<div class="card-body">
                			<div class="table-responsive">
                				<table id="example" class="table table-striped table-bordered second w-100" >
                					...
                					<tbody>
                						<tr>
                							<td class="text-center"> {{$topic->name_topic}}</td>
                							...
                							{{-- <td class="text-center"><a class="text-primary" href="{{ route('xemdetai',$topic->id) }}">(Tải về)</a></td> --}}
                                            ...
                                            <td class="text-center">
                                                @if($topic->getRegisterTopic->council == null)
                                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#sendgreedcouncil" data-id="{{$topic->id}}" data-name="{{$topic->name_topic}}">Giới thiệu hội đồng nghiệm thu</button>
                                                @else
                                                <button type="button" class="btn btn-success" data-toggle="modal" data-target="#listgreedcouncil" data-id="{{$topic->id}}" data-name="{{$topic->name_topic}}">
